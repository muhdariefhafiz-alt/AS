...
export default async function LawyerProfilePage({ params }: Props) {
  ...
  return (
    <>
      ...
      {/* Hero */}
      <section className="border-b border-gray-100 bg-gradient-to-b from-coral-50/40 to-white">
        <div className="mx-auto max-w-[1120px] px-5 pb-8 pt-8 md:px-8">
          <div className="flex items-start gap-4">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-coral-100 text-xl font-bold text-coral-700">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {lawyer.primary_firm && (
                  <Link href={`/lawyers/firm/${firms.length > 0 ? slugify(lawyer.primary_firm) : ''}`} className="text-coral-600 hover:underline">
                    {lawyer.primary_firm}
                  </Link>
                )}
            ...
            <div className="flex flex-col items-center rounded-xl border border-coral-200 bg-white px-4 py-3 shadow-sm">
              <span className="text-3xl font-extrabold text-coral-600">{lawyer.case_count}</span>
            ...
            {topAreas.length > 0 && (
              <section>
                ...
                <div className="mt-4 space-y-2">
                  {topAreas.map(area => (
                    <div key={area.name} className="flex items-center justify-between rounded-lg border border-gray-100 bg-white p-3">
                        ...
                        {c.case_types && c.case_types.length > 0 && (
                          <div className="mt-2 flex flex-wrap gap-1">
                            {c.case_types.filter((t: string) => !t.match(/^\d/) && t.length > 3).slice(0, 3).map((t: string) => (
                              <span key={t} className="rounded bg-gray-100 px-2 py-0.5 text-[10px] text-gray-500">{t.split(' — ')[0]}</span>
                            ))}
                          </div>
                        )}
                      </div>
                      <span className="shrink-0 rounded bg-coral-50 px-2 py-0.5 text-xs font-medium text-coral-600">{c.year}</span>
                    </div>
                  </div>
                ))}
                {caseList.length > 30 && (
