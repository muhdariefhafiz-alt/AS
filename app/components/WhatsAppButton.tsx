...
export default function WhatsAppButton({ whatsapp, agentId, agentSlug }: { whatsapp: string; agentId: number; agentSlug: string }) {
  const handleClick = () => {
    fetch("/api/funnel", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ event: "whatsapp_click", agentId, metadata: { slug: agentSlug } }),
    }).catch(() => {});
  };
