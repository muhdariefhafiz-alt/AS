...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${agency.name} has ${agency.agent_count.toLocaleString()} registered agents in Singapore. ${scoreText}${ratingText}Compare with other agencies on AgentScan.`,
  };
}
...
export default async function AgencyPage({ params }: Props) {
  ...
  const breadcrumbLd = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Home", item: "https://agentscan.sg" },
      { "@type": "ListItem", position: 2, name: "Agencies", item: "https://agentscan.sg/agencies" },
              ...
              <div className="flex items-start justify-between">
                ...
                {agency.score && (
                  <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-center">
                    <div className="text-xl font-bold text-emerald-700">{Math.round(Number(agency.score))}</div>
                    <div className="text-[8px] uppercase text-emerald-600">Score</div>
                  </div>
                )}
              </div>
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-2">
                  {agentList.map((agent) => (
                    <Link
                      ...
                      className="group flex items-center gap-3 rounded-lg bg-gray-50 p-3 transition hover:bg-emerald-50"
                    >
                      <div className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-100 text-sm font-bold text-emerald-700">
                        {agent.name.split(" ").map((n: string) => n[0]).join("").slice(0, 2)}
                      </div>
                      <div>
                        <div className="text-sm font-medium text-gray-900 group-hover:text-emerald-600">{agent.name}</div>
              ...
              <dl className="mt-4 space-y-3 text-sm">
                ...
                {agency.phone && (
                  <div className="flex justify-between">
                    ...
                    <dd><a href={`tel:${agency.phone}`} className="font-medium text-emerald-600">{agency.phone}</a></dd>
                  </div>
                )}
                {agency.website && (
                  <div className="flex justify-between">
                    <dt className="text-gray-500">Website</dt>
                    <dd><a href={agency.website} target="_blank" rel="noopener noreferrer" className="font-medium text-emerald-600 truncate block max-w-[150px]">{agency.website.replace(/^https?:\/\/(www\.)?/, "")}</a></dd>
                  </div>
                )}
              </dl>
            </div>
